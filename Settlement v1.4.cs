                ...
                using (FileStream fs = new FileStream(filePath, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (int row = 1; row < rowCount; row++)
                    {
                        try
                        {
                            ...
                            // 파일명 유효성 검사
                            if (string.IsNullOrWhiteSpace(rowData[0]) || string.IsNullOrWhiteSpace(rowData[1]) ||
                                string.IsNullOrWhiteSpace(rowData[2]) || string.IsNullOrWhiteSpace(rowData[3]) ||
                                string.IsNullOrWhiteSpace(rowData[5]))
                            {
                                progressBar.Value++; // 필수 데이터가 비어 있으면 진행바만 업데이트하고 파일 생성 건너뜀
                                continue;
                            }
                        ...
                        {
                            // 각 행의 예외를 개별적으로 처리하고 진행바를 업데이트
                            MessageBox.Show($"행 {row} 처리 중 오류 발생: {ex.Message}");
                            progressBar.Value++;
                        }
                    }
                }
